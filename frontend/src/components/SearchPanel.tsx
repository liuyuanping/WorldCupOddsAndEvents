...
export default function SearchPanel() {
  ...
  const doSearch = useCallback(async () => {
    ...
    setMsg("");
    try {
      const r = await fetch(`/api/v1/champion/search?team_ids=${[...selectedTeamIds].join(",")}&limit=8`);
  ...
  const selectResult = (r: SearchResult) => {
    setSelected(r);
    setEditTeamId(r.team_id);
    setEditTeamName(r.team_name);
    ...
    setEditConfidence(r.confidence);
          ...
          <div className="edit-row-inline">
            <div style={{ flex: 1 }}>
              ...
              <input type="datetime-local" value={editTs} onChange={(e) => setEditTs(e.target.value)}
                style={{ padding: "0.2rem 0.4rem", background: "var(--bg)", color: "var(--text)",
                  border: "1px solid var(--border)", borderRadius: "3px", fontSize: "0.7rem", width: "100%" }} />
            </div>
          </div>

          <div className="edit-row-inline">
            <div>
              <span className="edit-label">置信度</span>
              <input type="number" min="0" max="1" step="0.05" value={editConfidence}
                onChange={(e) => setEditConfidence(Number(e.target.value))}
                style={{ width: "70px", padding: "0.2rem 0.4rem", background: "var(--bg)",
                  color: "var(--text)", border: "1px solid var(--border)", borderRadius: "3px", fontSize: "0.7rem" }} />
            </div>
          </div>
